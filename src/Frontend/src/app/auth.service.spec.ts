...
describe('AuthService', () => {
  ...
  beforeEach(() => {
    ...
    cognito.getCurrentUser.and.rejectWith(
      Object.assign(new Error(), { name: 'UserUnAuthenticatedException' }),
    );
    TestBed.configureTestingModule({
      providers: [{ provide: CognitoAuthClient, useValue: cognito }],
    });
  ...
  it('configures Cognito without opening a hosted login page', async () => {
    ...
    expect(cognito.configure).toHaveBeenCalledWith(
      'eu-central-1_test',
      'test-client',
    );
  ...
  it('restores an existing session after a page refresh', async () => {
    cognito.getCurrentUser.and.resolveTo({
      username: 'person',
      userId: 'user-a',
    });
  ...
  it('opens confirmation for an unconfirmed account', async () => {
    cognito.signIn.and.rejectWith(
      Object.assign(new Error(), { name: 'UserNotConfirmedException' }),
    );
    await auth.initialize();
    expect(await auth.signIn('person@example.test', 'Password1234')).toBe(
      'CONFIRM_SIGN_UP',
    );
  });
  ...
  it('opens password recovery when Cognito requires a reset', async () => {
    cognito.signIn.and.rejectWith(
      Object.assign(new Error(), { name: 'PasswordResetRequiredException' }),
    );
    await auth.initialize();
    expect(await auth.signIn('person@example.test', 'Password1234')).toBe(
      'RESET_PASSWORD',
